...
const API_KEY = process.env.NEXT_PUBLIC_YOUTUBE_API_KEY; // Ensure API key is set in environment variables
...
export default function RecentLecture() {
  ...
  // Function to fetch the latest video data from YouTube
  ...
  // Function to fetch video statistics (e.g., views count)
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 shadow-custom1 gap-4">
          {lectures.map((lecture, index) => (
            ...
            >
              ...
              <h3 className="relative text-[20px] cursor-pointer  font-bold mt-4 mb-6 px-4 line-clamp-2 after:content-['']  ">
                {lecture.title}
              </h3>
              <div className="flex justify-between w-full px-4 mb-4 mt-2 text-sm text-gray-500 px-2">
