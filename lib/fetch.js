...
export const getHomeLectures = async () => {
	...
	return {
		videoLists: lectures.videoLists.videos.slice(4, 8),
...
export const getAllPlaylists2 = async () => {
	...
	if (!data || !data.items || !Array.isArray(data.items)) {
	  ...
	  return { playlists: [], playlistsTitle: {} }; // Handle unexpected response
	}
	...
	if (total > 50) {
	  ...
	  for (let i = 1; i < numberOfRequests; i++) {
		...
		if (!data || !data.items || !Array.isArray(data.items)) {
		  ...
		  continue; // Skip to the next iteration if the response is not as expected
		}
