      ...
      <div className="container mx-auto">
        ...
        <div className="team__slider swiper min-h-[500px]">
          <div className="swiper-wrapper">
            {/* Mapping over the teamMembers in pairs of two */}
            {teamMembers.map((member, index) => {
              // Only render a slide when index is even, and the next item exists
              if (index % 2 === 0) {
                return (
                    ...
                    <div className="flex flex-col md:flex-row gap-9">
                      ...
                      <div className="flex-1 flex flex-col xl:flex-row items-center gap-[30px]">
                        ...
                        <div className="flex-1 flex flex-col">
                          <h4 className="h4 text-[#4C5354] font-bold text-2xl mb-[8px]">{teamMembers[index].name}</h4>
                          <div className="text-[#9AB4B7] text-[22px] mb-[12px] max-w-[320px]">{teamMembers[index].position}</div>
                          <p className="font-light text-[#777F81] text-[20px] mb-[26px] max-w-[320px]">{teamMembers[index].description}</p>
                      ...
                      {teamMembers[index + 1] && (
                        <div className="flex-1 flex flex-col xl:flex-row items-center gap-[30px]">
                          ...
                          <div className="flex-1 flex flex-col">
                            <h4 className="h4 text-[#4C5354] font-bold text-2xl mb-[8px]">{teamMembers[index + 1].name}</h4>
                            <div className="text-[#9AB4B7] text-[22px] mb-[12px] max-w-[320px]">{teamMembers[index + 1].position}</div>
                            <p className="font-light text-[#777F81] text-[20px] mb-[26px] max-w-[320px]">{teamMembers[index + 1].description}</p>
                            <div className="flex items-center text-[30px] gap-x-5 text-accent-tertiary">
                              <a href="#" className="cursor-pointer hover:text-accent transition-all">
                                <i className="ri-youtube-fill"></i>
                              </a>
